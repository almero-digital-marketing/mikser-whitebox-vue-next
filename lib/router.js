export default async (app, { router, store }) => {
	...
	router.beforeEach((to, from, next) => {
		...
		let documentRefId = decodeURI(to.path)
		let documentRoute = router.documentRoutes[documentRefId]
		if (documentRoute) {
			const existingDocument = store.getters['mikser/href'](documentRoute.href, documentRoute.lang, true)
			if (existingDocument) {
				return next()
			}
		}
		store.dispatch('mikser/init', to.path).then(() => {
			next()
		})
	})
