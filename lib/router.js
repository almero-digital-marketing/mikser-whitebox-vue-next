export default (app, { router, store }) => {
	router.documentRoutes = router.documentRoutes || {}
	router.reverseRoutes = router.reverseRoutes || {}

	router.beforeEach((to, from, next) => {
		window.document.documentElement.lang = to.params.lang || window.document.documentElement.lang
		let documents = []
		let documentRoute = router.documentRoutes[decodeURI(to.path)]
		if (documentRoute) {
		...
		for(let matched of to.matched) {
			...
			if (matched.meta.refId) {
				documentRoute = router.documentRoutes[matched.meta.refId]
        ...
        .then(() => {
			if (data.length) {
				...
				store.dispatch('mikser/init', documents)
				.then(items => store.dispatch('take', items))
				.then(() =>	next())
			} else {
				next()
			}
