		...
		actions: {
			init({ commit, state, getters }, items) {
				...
				if (typeof items == 'string') {
					path = decodeURI(items)
					items = []
					let documentRoute = router.documentRoutes[path]
					if (documentRoute) {
						const existingDocument = getters.href(documentRoute.href, documentRoute.lang, true)
						if (existingDocument) {
				...
				if (!items) items = []
 				return new Promise(resolve => {
					window.whitebox.init('feed', (feed) => {
						let loading = []
						let route = router.documentRoutes[decodeURI(path)]
						let refIds = []
						...
						let documentRefId = decodeURI(path)
