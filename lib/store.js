...
export default (app, { router, store }) => {
	store.registerModule('mikser', {
		...
		getters: {
			currentRefId: () => {
				return router.currentRoute.value.meta.refId || decodeURI(router.currentRoute.value.path)
			},
			...
			document: (state, getters) => {
				let route = router.documentRoutes[getters.currentRefId]
				if (!route) return
				let document = getters.href(route.href, route.document.meta.lang)
				document.route = route
				return document	
			},
			href: (state, getters) => (href, lang, loaded) => {
				...
				lang =
					lang ||
					router.currentRoute.value && (router.documentRoutes[getters.currentRefId] && router.documentRoutes[getters.currentRefId].document.meta.lang) ||
			...
			hrefs: (state, getters) => (regex, lang, loaded) => {
				if (typeof lang == 'boolean') {
					loaded = lang
					lang = undefined
				}
				...
				lang =
					lang ||
					(router.documentRoutes[getters.currentRefId] && router.documentRoutes[getters.currentRefId].document.meta.lang) ||
					document.documentElement.lang ||
					''
				let hreflang = state.sitemap[lang]
				if (hreflang) {
		...
		mutations: {
			...
			setHand(state, items) {
                if (items) {
                    state.hand.push(...items.map(item => Object.freeze(item)))
                } else {
                    state.hand = []
                }
            }
								...
								if (route) {
									if (router.reverseRoutes[item]) {
										...
										if (feedPool[documentRefId] == undefined ) {
											let documentRoute = router.documentRoutes[documentRefId]
