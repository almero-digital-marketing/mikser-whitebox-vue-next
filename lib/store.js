...
function mikserStore(app, { router, store, domainConfig }) {
	store.registerModule('mikser', {
		...
		getters: {
			...
			document: (state, getters) => {
				let route = domainConfig.documentRoutes[getters.currentRefId]
				if (!route) return
			...
			href: (state, getters) => (href, lang, loaded) => {
				...
				lang =
					lang ||
					router.currentRoute.value && (domainConfig.documentRoutes[getters.currentRefId] && domainConfig.documentRoutes[getters.currentRefId].document.meta.lang) ||
			...
			hrefs: (state, getters) => (regex, lang, loaded) => {
				...
				lang =
					lang ||
					(domainConfig.documentRoutes[getters.currentRefId] && domainConfig.documentRoutes[getters.currentRefId].document.meta.lang) ||
					document.documentElement.lang ||
					''
				let hreflang = state.sitemap[lang]
				if (hreflang) {
